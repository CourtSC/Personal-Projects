...
for file in os.listdir():
    if '.zip' in file:
        with zipfile.ZipFile(file, 'r') as zip:
            zip.extractall()
        os.remove(file) # Writing a new zip file later in this program will automatically overwrite zip files with the same name as the parent folder, but if another .zip file is present then this line will delete it when we're done with it.
